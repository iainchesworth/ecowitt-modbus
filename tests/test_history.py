...
async def test_decodes_most_recent_minute(ws90: WS90, unit: MockModbusUnit) -> None:
    ...
    await ws90.history.async_update()

    assert len(ws90.history.max_light) == 30
    assert ws90.history.max_light[0].value == 17670
    assert ws90.history.battery_voltage == pytest.approx(3.2)
    assert ws90.history.capacitance_voltage == pytest.approx(5.0)
...
async def test_thirtieth_sample_is_thirty_minutes_ago(
    ...
    assert ws90.history.avg_temperature[0].value == pytest.approx(-40.0)
    assert ws90.history.avg_temperature[29].value == pytest.approx(29 * 0.1 - 40)
...
async def test_rainfall_sentinel_decodes_to_none(
    ...
    assert ws90.history.rainfall[0].value is None
